var firebaseConfig = {
  ...
  let post3=localStorage.getItem("query");
...
var key=post3.key;
  ...
  $("#submitComment").click(function (){
      ...
      if(!user)
      {
        alert("please login");
         ...
         var com=firebase.database().ref('posts/'+key).child("comment");
         var newCom=com.push();
         
                firebase.database().ref('posts/'+key+'/comment/'+newCom.key).update({
                    comment:body,
                    name:name,
                });
               alert("comment added");
  ...
  var commentRef=firebase.database().ref('posts/'+key).child("comment");
